    ...
    * `continuous=True` converts the environment to use discrete action space.
    ...
    def _render(self, mode: str):
        ...
        if mode == "human":
            ...
            img = self._create_image_array(self.surf, (STATE_W, STATE_H))  # Get the image
            crop_height = int(STATE_H * 0.85)  # Keep top 85% (adjust this if needed)
    
            img_cropped = img[:crop_height, :, :]  # Crop the bottom part
            img_resized = cv2.resize(img_cropped, (STATE_W, STATE_H), interpolation=cv2.INTER_AREA)  # Resize back to (96,96)
