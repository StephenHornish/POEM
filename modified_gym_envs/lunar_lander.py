...
class LunarLander(gym.Env, EzPickle):
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def step(self, action):
        ...
        m_power = 0.0
        if self.fuel > 0 and (self.continuous and action[0] > 0.0) or (
        ...
        reward -= s_power * 0.03
        # Penalize firing engines when basically stationary
        if (
            abs(vel.x) < 0.01 and abs(vel.y) < 0.01  # very low velocity
            and (m_power > 0.0 or s_power > 0.0)      # thrust is happening
        ):
            reward -= 0.5  # or some reasonable penalty
    ...
    def render(self):
        ...
        if self.render_mode == "human":
            ...
            fuel_text = f"Fuel: {int(self.fuel)}"
            fuel_surface = self.font.render(fuel_text, True, (255, 255, 255))  # white text
            self.surf.blit(fuel_surface, (10, 10))  # top-left corner
